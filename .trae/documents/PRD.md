...
- 采用前后端分离架构，前端负责界面渲染与交互，后端负责业务逻辑与数据持久化。
...
用户首次访问系统时进入登录页，输入账号密码后后端进行 JWT 认证；认证成功返回长时效 Token，前端保存并拉取用户信息、角色、权限与菜单树；之后用户根据权限访问对应页面与按钮，所有管理操作与查询均被记录到操作日志。

```mermaid
graph TD
    A["用户访问后台地址"] --> B["路由守卫检查 Token"]
    B -->|无 Token| C["跳转登录页"]
    C --> D["输入账号密码"]
    D --> E["后端 JWT 认证"]
    E -->|认证失败| F["提示错误/账号锁定"]
...
- **组件风格**：采用 Element Plus 组件库，按钮圆角 `4px`，表单统一 `max-width` 限制，表格行高 `48px`。
